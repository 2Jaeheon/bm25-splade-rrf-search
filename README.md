...
### 배경
- **BM25만 사용할 때**: 정확한 단어 매칭에 강하지만, 동의어(예: "car" vs "automobile")를 놓치기 쉽습니다.
- **신경망 검색(SPLADE)만 사용할 때**: 의미적 유사성을 잘 포착하지만, 특수 용어, 숫자, 정확한 키워드 매칭에서는 BM25보다 약할 수 있습니다.
- **결론**: 두 접근법의 장점을 결합하면 검색 품질이 향상되기 때문에 (BM25 + SPLADE 하이브리드)에 따라 본 프로젝트를 설계하였습니다.

### 기술 선택 이유
- **RRF(Reciprocal Rank Fusion)**: BM25와 SPLADE의 점수 스케일이 다르므로, 별도의 가중치 튜닝 없이 랭크 기반으로 안정적으로 섞을 수 있어 RRF를 선택하였습니다.
- **SPLADE**: Dense retrieval(임베딩 + 코사인 유사도) 대신 sparse retrieval을 쓰면 기존 역색인 인프라와 호환되고, 해석 가능한 토큰 가중치를 얻을 수 있을 것이라고 예측해서 선택하였습니다..
- **Doc2Query(문서 확장)**: 짧거나 키워드만 있는 문서는 쿼리와 매칭되기 어려우므로, Doc2Query로 문서를 확장해 검색 가능성을 높였습니다.
